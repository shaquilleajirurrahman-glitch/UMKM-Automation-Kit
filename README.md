...
The scraper returns an empty list (`[]`) — rather than raising an exception — when scraping yields zero results (e.g., site down, HTML structure changed). The pipeline explicitly checks for this case (`if not data:`) and logs it as a `FAILED` run. Relying on `try/except` alone would have silently treated "0 products scraped, no errors thrown" as a successful run.
